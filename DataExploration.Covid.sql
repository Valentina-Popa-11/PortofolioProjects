...
SELECT  
	SUM(new_cases) AS reported_cases, 
...
-- Showing Covid effect per continent
SELECT 
	continent, 
	SUM(new_cases) AS reported_cases, 
	SUM(CAST(new_deaths as int)) as total_deaths, 
...
DROP TABLE 
	IF EXISTS #PercentEuropeanPopulationVaccinated
CREATE TABLE 
	#PercentEuropeanPopulationVaccinated
(
...
SELECT 
	...
	cd.population, 
	cv.new_vaccinations, 
	SUM(CAST(cv.new_vaccinations AS BIGINT)) --BIGINT is used as result of calculation is outside the range of int data type
...
SELECT 
	*, 
	(Rolling_people_vaccinated/Population)*100 AS PercentVaccinated
FROM
	#PercentEuropeanPopulationVaccinated
ORDER BY 
